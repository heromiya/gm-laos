...
gdalbuildvrt -overwrite -input_file_list tmp.txt tmp.vrt
gdal_translate -co compress=deflate tmp.vrt tmp_bing.tif

XMIN=`gdalinfo tmp_bing.tif | grep "Upper Left" | sed 's/^Upper Left  (\([-.0-9]*\), \([-.0-9]*\)) .*/\1/g'`
YMIN=`gdalinfo tmp_bing.tif | grep "Lower Right"| sed 's/^Lower Right (\([-.0-9]*\), \([-.0-9]*\)) .*/\2/g'`
XMAX=`gdalinfo tmp_bing.tif | grep "Lower Right"| sed 's/^Lower Right (\([-.0-9]*\), \([-.0-9]*\)) .*/\1/g'`
YMAX=`gdalinfo tmp_bing.tif | grep "Upper Left" | sed 's/^Upper Left  (\([-.0-9]*\), \([-.0-9]*\)) .*/\2/g'`
XRES=`gdalinfo tmp_bing.tif | grep "Pixel Size" | sed 's/Pixel Size = (\([-.0-9]*\),\([-.0-9]*\))/\1/'`
YRES=`gdalinfo tmp_bing.tif | grep "Pixel Size" | sed 's/Pixel Size = (\([-.0-9]*\),\([-.0-9]*\))/\2/'`
